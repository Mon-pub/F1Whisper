...
final class UnreadDividerLocator {
    ...
    static final class MessageFlags {
         ...
         */
        static final MessageFlags NOT_UNREAD = new MessageFlags(false, false, false, false);
        ...
        MessageFlags(boolean outbox, boolean read, boolean statusMessage, boolean saved) {
            this.outbox = outbox;
            this.read = read;
         ...
         * ({@code !outbox}), saved, not-yet-read, non-status message.
         */
        boolean isUnread() {
            return !outbox && saved && !read && !statusMessage;
        }
    }
