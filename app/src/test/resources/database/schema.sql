-- This file was generated by DatabaseSchemaCreatorTest on 2026-08-02T19:48:36.109745155Z. --
...
CREATE INDEX `m_group_message_expiresAt_idx` ON `m_group_message` ( `expiresAtUtc` );
...
CREATE TABLE IF NOT EXISTS `chat_folder` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `name` VARCHAR NOT NULL, `position` INTEGER NOT NULL, `createdAt` BIGINT);
CREATE TABLE IF NOT EXISTS `chat_folder_member` (`folderId` INTEGER NOT NULL, `conversationUid` VARCHAR NOT NULL, `createdAt` BIGINT, PRIMARY KEY (`folderId`, `conversationUid`), FOREIGN KEY(`folderId`) REFERENCES `chat_folder`(`id`) ON UPDATE CASCADE ON DELETE CASCADE);
